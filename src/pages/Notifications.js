...
export default function Notifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('notifications')
      .select('*, actor:users!notifications_actor_id_fkey(username, avatar_url), prompt:prompts(title)')
      .eq('user_id', user.id)
            ...
            const row = (
              <div style={{ ...styles.row, background: n.is_read ? 'transparent' : 'rgba(0,229,255,0.05)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={styles.text}>
                    <span style={styles.actorName}>{n.actor?.username || 'Someone'}</span> {messageFor(n)}
                  </p>
