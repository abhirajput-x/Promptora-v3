...
import { X, Copy, Check, Heart, Bookmark, Share2, ChevronRight, Eye } from 'lucide-react';
import { supabase, logPromptView } from '../lib/supabase';
...
export default function PromptDetail({ prompt, onClose }) {
  ...
  const [viewsCount, setViewsCount] = useState(prompt?.views_count || 0);
  ...
  useEffect(() => {
    ...
    setViewsCount(prompt?.views_count || 0);
  }, [prompt]);

  useEffect(() => {
    if (!prompt?.id) return;
    logPromptView(prompt.id, user?.id).then(() => {
      setViewsCount(v => v + 1); // optimistic — server dedupes silently if already counted today
    });
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 600 }}>
        ...
        <div style={styles.meta}>
          ...
          <span style={styles.viewsTag}><Eye size={12} /> {viewsCount}</span>
        </div>
          ...
          <Link to={`/creator/${prompt.user_id}`} onClick={onClose} style={styles.creator}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, fontWeight: 600 }}>
                ...
                {prompt.creator.is_verified && <VerifiedBadge size={13} />}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 3 }}>
                <span style={{ fontSize: 12, color: '#888' }}>Creator</span>
...
const styles = {
  ...
  meta: { display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14, alignItems: 'center' },
  viewsTag: {
    display: 'flex', alignItems: 'center', gap: 4,
    fontSize: 11.5, color: '#666', fontWeight: 600, marginLeft: 2,
  },
  promptBox: {
